...
# 2 load generated TTS segment audio files
# 3 optionally extract background music from original audio
# 4 create silent timeline
# 5 place segment audio at correct timestamps
# 6 merge background music (optional)
# 7 export final dubbed audio
# ------------------------------------------------
...
def get_audio_duration(audio_file: str) -> int:
    ...
    if result.returncode != 0:
        raise RuntimeError(f"Failed to get duration for {audio_file}")
...
def build_final_audio(
    ...
    """
    Build final dubbed audio from generated TTS segments.
    """
    ...
    # ------------------------------------------------
    # 2 Optional background music extraction
    # ------------------------------------------------
    ...
    if use_background_music:
        try:
            ...
            # reduce background volume slightly
            background_music = background_music - 26

        except Exception as e:
            print("Background music extraction failed:", e)
            print("Continuing without background music.")
            background_music = None

    # ------------------------------------------------
    # 3 Create silent timeline
    # ------------------------------------------------
    duration_ms = get_audio_duration(original_audio_path)

    tts_track = AudioSegment.silent(duration=duration_ms)

    # Ensure background track matches video length
    if background_music is not None and len(background_music) < duration_ms:
        background_music += AudioSegment.silent(duration_ms - len(background_music))

    # ------------------------------------------------
    # 4 Insert TTS segments into timeline
    # ------------------------------------------------
    ...
    for i, segment in enumerate(transcript):
        ...
        if not segment_file.exists():
            print(f"Missing segment file: {segment_file}")
            continue
